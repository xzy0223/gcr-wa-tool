...
2. the fields [pillar, question, choice, ta_check] have definite ones in <schema> tag. the output should comprise two field, one resource name or aws arn of the finding corresponding to "ta_check", which needs you to search in the "description" field; the other is the AWS service name of the resource, and you can also refer the information in "description" field. please respect the output format as <output_format> tag, put the service name into the child tag <service_name>, and put the resource name into child tag <resource>
3. When you searching corresponding resource, if there is no aws arn in a row in csv data, please try you best to search the specific item name based on description of field "ta check". Please noted, DO NOT combine field values, just choosing a field value to use. 
    3.1 if the "ta_check" is related to IAM access key, MUST choose the IAM user name
